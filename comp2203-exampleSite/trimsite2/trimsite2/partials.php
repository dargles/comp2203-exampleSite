 ...
 */
  ...
  class partials
  {
	...
	private   $path = "trimsite2/";
	
	public function __construct()
    {
      if(@parse_ini_file("page.ini",true)){
        $iniFile = (object) @parse_ini_file("page.ini",true);
        $this->template = $iniFile->template;
        $this->css = $iniFile->css;
    ...
    public function render($partial, $substitutions = array()) 
	  ...
	  //set the required template for the partial
	  $partial = $this->template.$partial;
      // Check that the partial exists
      $filename = $this->path."partials/$partial.html";
      if(file_exists($filename))
      {
        ...
        else $template = "Couldn't find $partial in $filename"; 
